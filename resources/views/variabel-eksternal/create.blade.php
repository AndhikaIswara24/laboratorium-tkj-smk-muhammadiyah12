@extends('layouts.app')
@section('title', 'Tambah Observasi Variabel Eksternal')
@section('content')
        ...
        <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
            ...
            <div class="border-b border-slate-200 bg-gradient-to-r from-blue-600 to-indigo-600 px-6 py-5 dark:border-slate-800">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h1 class="text-lg font-bold text-white">Tambah Observasi Variabel Eksternal</h1>
                        <p class="text-sm text-blue-100">Catat kondisi eksternal seperti lingkungan, pasokan daya, sparepart, anggaran, dan efek eksternal untuk aset</p>
                    </div>
                </div>
            </div>
            ...
            <div class="p-6">
                ...
                <form action="{{ route('variabel.store') }}" method="POST" id="formVariabel">
                    @csrf
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <select id="id_aset" name="id_aset" required
                                    ...
                                    @foreach($assets as $aset)
                                        <option value="{{ $aset->id_aset }}" @selected(old('id_aset') == $aset->id_aset)>
                        ...
                        <div class="grid gap-5 sm:grid-cols-2">
                            ...
                            <div>
                                ...
                                <select id="lingkungan" name="lingkungan" required class="form-control w-full py-2.5">
                                    ...
                                    <option value="Baik" @selected(old('lingkungan') === 'Baik')>Baik (Bersih, suhu terjaga, sirkulasi lancar)</option>
                                    <option value="Cukup" @selected(old('lingkungan') === 'Cukup')>Cukup (Berdebu sedang, suhu ruang wajar)</option>
                                    <option value="Buruk" @selected(old('lingkungan') === 'Buruk')>Buruk (Sangat berdebu, lembab, atau suhu panas)</option>
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <select id="daya_listrik" name="daya_listrik" required class="form-control w-full py-2.5">
                                    <option value="">-- Pilih Penilaian --</option>
                                    <option value="Stabil" @selected(old('daya_listrik') === 'Stabil')>Stabil (Goncangan daya minim/tidak ada pemadaman)</option>
                                    <option value="Tidak Stabil" @selected(old('daya_listrik') === 'Tidak Stabil')>Tidak Stabil (Voltase naik turun, jarang padam)</option>
                                    <option value="Sering Padam" @selected(old('daya_listrik') === 'Sering Padam')>Sering Padam (Aliran listrik sering mati mendadak)</option>
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <select id="sparepart" name="sparepart" required class="form-control w-full py-2.5">
                                    <option value="">-- Pilih Penilaian --</option>
                                    <option value="Tersedia" @selected(old('sparepart') === 'Tersedia')>Tersedia (Suku cadang mudah diperoleh/banyak stok)</option>
                                    <option value="Terbatas" @selected(old('sparepart') === 'Terbatas')>Terbatas (Inden/sulit didapat dalam waktu cepat)</option>
                                    <option value="Tidak Ada" @selected(old('sparepart') === 'Tidak Ada')>Tidak Ada (Langka/tidak diproduksi lagi)</option>
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <select id="anggaran" name="anggaran" required class="form-control w-full py-2.5">
                                    <option value="">-- Pilih Penilaian --</option>
                                    <option value="Mendukung" @selected(old('anggaran') === 'Mendukung')>Mendukung (Dana operasional/perawatan lancar)</option>
                                    <option value="Terbatas" @selected(old('anggaran') === 'Terbatas')>Terbatas (Harus pengajuan bertahap/dana minim)</option>
                                    <option value="Tidak Ada" @selected(old('anggaran') === 'Tidak Ada')>Tidak Ada (Sama sekali tidak ada dana khusus)</option>
                                </select>
                            </div>
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <select id="ext_effect" name="ext_effect" required class="form-control w-full py-2.5">
                                    ...
                                    <option value="Rendah" @selected(old('ext_effect') === 'Rendah')>Rendah (Faktor luar hampir tidak mengganggu fungsi aset)</option>
                                    <option value="Sedang" @selected(old('ext_effect') === 'Sedang')>Sedang (Faktor luar sesekali memicu kelambatan/kerusakan)</option>
                                    <option value="Tinggi" @selected(old('ext_effect') === 'Tinggi')>Tinggi (Faktor luar sangat mengancam kelangsungan hidup aset)</option>
                                </select>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-end gap-3 border-t border-slate-200 pt-6 dark:border-slate-800">
                        ...
                        <button type="submit" class="btn-primary">
                            <i class="fa-solid fa-floppy-disk"></i> Simpan Observasi
                        </button>
                    </div>
                </form>
            </div>
