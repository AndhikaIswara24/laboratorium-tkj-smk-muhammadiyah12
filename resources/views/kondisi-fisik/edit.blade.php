@extends('layouts.app')
@section('title','Edit Observasi Kondisi Fisik')
@section('content')
...
<div class="space-y-6">
    ...
    <div class="mx-auto max-w-4xl">
        <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
            {{-- Card Header --}}
            <div class="border-b border-slate-200 bg-gradient-to-r from-amber-500 to-orange-500 px-6 py-5 dark:border-slate-800">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h1 class="text-lg font-bold text-white">Edit Data Observasi</h1>
                        <p class="text-sm text-amber-100">
                            Perbarui data kondisi fisik & teknis —
                            <strong>{{ $kondisi->asset->kode_brg ?? '' }}</strong>
                            {{ $kondisi->asset->nama_brg ?? '' }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="p-6">
                ...
                <form action="{{ route('kondisi.update', $kondisi->id_kondisi) }}" method="POST" id="formKondisiFisikEdit">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-8">
                        ...
                        <div class="grid gap-5 sm:grid-cols-2">
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <select id="id_aset_select" name="id_aset_select" required disabled
                                    ...
                                    @foreach($assets as $aset)
                                        <option value="{{ $aset->id_aset }}" data-thn="{{ $aset->thn_perolehan }}" data-kode="{{ $aset->kode_brg }}" @selected(old('id_aset', $kondisi->id_aset) == $aset->id_aset)>
                                            {{ $aset->kode_brg }} — {{ $aset->nama_brg }} ({{ $aset->merk_tipe ?? '-' }})
                                        </option>
                                    @endforeach
                                </select>
                                <input type="hidden" name="id_aset" value="{{ $kondisi->id_aset }}">
                            ...
                            {{-- Asset Info Card --}}
                            <div id="assetInfoCard" class="sm:col-span-2">
                                <div class="rounded-xl border border-blue-200 bg-blue-50 p-4 dark:border-blue-500/20 dark:bg-blue-500/10">
                                    <div class="grid gap-3 sm:grid-cols-3">
                                        <div>
                                            ...
                                            <p id="infoKode" class="mt-0.5 text-sm font-bold text-blue-900 dark:text-blue-100">{{ $kondisi->asset->kode_brg ?? '-' }}</p>
                                        </div>
                                        <div>
                                            <p class="text-xs font-semibold uppercase text-blue-600 dark:text-blue-300">Tahun Perolehan</p>
                                            <p id="infoTahun" class="mt-0.5 text-sm font-bold text-blue-900 dark:text-blue-100">{{ $kondisi->asset->thn_perolehan ?? 'Tidak diketahui' }}</p>
                                        </div>
                                        <div>
                                            ...
                                            <p id="infoUsia" class="mt-0.5 text-sm font-bold text-blue-900 dark:text-blue-100">
                                                <span id="usiaValue">{{ $kondisi->usia_pakai ?? 0 }}</span> <span class="text-xs font-normal">tahun</span>
                                            </p>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                            ...
                            <div>
                                ...
                                <input type="date" id="tgl_observasi" name="tgl_observasi"
                                       value="{{ old('tgl_observasi', $kondisi->tgl_observasi ? $kondisi->tgl_observasi->format('Y-m-d') : '') }}" required
                            ...
                            <div>
                                ...
                                <select id="kondisi_brg" name="kondisi_brg" required
                                    ...
                                    <option value="B" @selected(old('kondisi_brg', $kondisi->kondisi_brg) === 'B')>B (Baik)</option>
                                    <option value="RR" @selected(old('kondisi_brg', $kondisi->kondisi_brg) === 'RR')>RR (Rusak Ringan)</option>
                                    <option value="RB" @selected(old('kondisi_brg', $kondisi->kondisi_brg) === 'RB')>RB (Rusak Berat)</option>
                                </select>
                                @error('kondisi_brg')
                                    <p class="mt-1 text-xs font-medium text-rose-600 dark:text-rose-400">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <select id="ket_teknis" name="ket_teknis" required
                                    ...
                                    <option value="Normal" @selected(old('ket_teknis', $kondisi->ket_teknis) === 'Normal')>Normal</option>
                                    <option value="Lemah" @selected(old('ket_teknis', $kondisi->ket_teknis) === 'Lemah')>Lemah</option>
                                    <option value="Lambat" @selected(old('ket_teknis', $kondisi->ket_teknis) === 'Lambat')>Lambat</option>
                                    <option value="Mati Total" @selected(old('ket_teknis', $kondisi->ket_teknis) === 'Mati Total')>Mati Total</option>
                                </select>
                                @error('ket_teknis')
                                    <p class="mt-1 text-xs font-medium text-rose-600 dark:text-rose-400">{{ $message }}</p>
                                @enderror
                    ...
                    <div class="mb-8">
                        ...
                        <div class="grid gap-5 sm:grid-cols-2">
                            ...
                            <div>
                                ...
                                @php
                                    $currentUsia = $kondisi->asset && $kondisi->asset->thn_perolehan
                                        ? (date('Y') - $kondisi->asset->thn_perolehan) . ' tahun (dari ' . $kondisi->asset->thn_perolehan . ')'
                                        : ($kondisi->usia_pakai ?? 0) . ' tahun';
                                @endphp
                            ...
                            <div>
                                ...
                                <input type="number" id="frq_kerusakan" name="frq_kerusakan"
                                       ...
                                       value="{{ old('frq_kerusakan', $kondisi->frq_kerusakan) }}" required
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <div class="grid grid-cols-3 gap-3">
                                    <label class="relative">
                                        <input type="radio" name="kelas_label" value="Layak" class="peer sr-only" @checked(old('kelas_label', $kondisi->kelas_label) === 'Layak') required>
                                    ...
                                    <label class="relative">
                                        <input type="radio" name="kelas_label" value="Perlu Servis" class="peer sr-only" @checked(old('kelas_label', $kondisi->kelas_label) === 'Perlu Servis')>
                                    ...
                                    <label class="relative">
                                        <input type="radio" name="kelas_label" value="Tidak Layak" class="peer sr-only" @checked(old('kelas_label', $kondisi->kelas_label) === 'Tidak Layak')>
                    ...
                    <div class="flex items-center justify-end gap-3 border-t border-slate-200 pt-6 dark:border-slate-800">
                        ...
                        <button type="submit" class="inline-flex items-center justify-center gap-2 rounded-lg bg-amber-500 px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-amber-600 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-2 dark:focus:ring-offset-slate-950">
