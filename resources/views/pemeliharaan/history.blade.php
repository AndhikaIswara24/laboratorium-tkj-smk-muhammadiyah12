@extends('layouts.app')
@section('title', 'Riwayat Pemeliharaan Aset')
@section('content')
...
<div class="space-y-6">
    ...
    <nav class="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400">
        ...
        <span class="font-medium text-slate-700 dark:text-slate-200">Riwayat Pemeliharaan</span>
    </nav>
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white">
                <i class="fa-solid fa-clock-rotate-left mr-2 text-blue-600"></i>Riwayat Pemeliharaan Aset
            </h1>
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
                Lacak seluruh pengerjaan servis dan perawatan preventif/korektif untuk aset ini.
            </p>
        </div>
        <div class="flex gap-2">
            ...
            <a href="{{ route('pemeliharaan.create', ['id_aset' => $asset->id_aset]) }}" class="btn-primary">
                <i class="fa-solid fa-plus"></i> Tambah Tindakan PM
            </a>
        </div>
    </div>
    ...
    <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <h3 class="mb-4 text-base font-bold text-slate-900 dark:text-white">
            <i class="fa-solid fa-list-ul mr-2 text-blue-600"></i>Daftar Riwayat Pemeliharaan
        </h3>
                ...
                <table class="w-full text-left text-sm">
                    <thead>
                        <tr class="border-b border-slate-200 bg-slate-50 dark:border-slate-800 dark:bg-slate-800/50">
                            ...
                            <th class="px-4 py-3 font-semibold text-slate-600 dark:text-slate-300">Jenis PM</th>
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                        @forelse($rows as $row)
                            <tr class="transition hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                ...
                                <td class="px-4 py-3">
                                    <span class="text-sm font-bold text-slate-700 dark:text-slate-200">{{ $row->interval_bulan ?? 0 }}</span>
                                    <span class="text-xs text-slate-400"> bulan</span>
                                </td>
                                <td class="px-4 py-3 text-slate-700 dark:text-slate-300 text-xs font-semibold">
                                    {{ $row->pelaksana ?? '-' }}
                                </td>
                                ...
                                <td class="px-4 py-3 text-center">
                                    <div class="inline-flex items-center gap-1">
                                        ...
                                        <button type="button" class="inline-flex h-8 w-8 items-center justify-center rounded-lg text-slate-500 transition hover:bg-rose-50 hover:text-rose-600 dark:hover:bg-rose-500/10 dark:hover:text-rose-400"
                                                onclick="openDeleteModal({{ $row->id_pm }})" title="Hapus">
                            ...
                            <tr>
                                <td colspan="9" class="px-4 py-12 text-center">
                                    <div class="flex flex-col items-center gap-3">
                                        ...
                                        <div>
                                            <p class="font-semibold text-slate-600 dark:text-slate-300">Belum ada riwayat pemeliharaan</p>
                                            <p class="text-sm text-slate-400">Klik "Tambah Tindakan PM" untuk mencatat pemeliharaan pertama aset ini.</p>
                                        </div>
                                    </div>
                                </td>
                            </tr>
...
<div id="deleteModal" class="fixed inset-0 z-[60] hidden items-center justify-center bg-slate-950/60 backdrop-blur-sm" style="display:none;">
    <div class="w-full max-w-md animate-[fadeInScale_0.2s_ease-out] rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl dark:border-slate-700 dark:bg-slate-900">
        ...
        <p class="mb-6 text-sm text-slate-600 dark:text-slate-300">
            Apakah Anda yakin ingin menghapus data pemeliharaan terpilih untuk aset <strong class="text-slate-900 dark:text-white">{{ $asset->nama_brg }}</strong>?
        </p>
