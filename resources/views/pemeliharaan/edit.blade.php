...
<div class="space-y-6">
    ...
    <nav class="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400">
        ...
        <span class="font-medium text-slate-700 dark:text-slate-200">Edit Pemeliharaan</span>
    </nav>
        ...
        <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
            ...
            <div class="border-b border-slate-200 bg-gradient-to-r from-amber-500 to-orange-600 px-6 py-5 dark:border-slate-800">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h1 class="text-lg font-bold text-white">Edit Data Pemeliharaan</h1>
                        <p class="text-sm text-amber-100">Perbarui catatan tindakan pemeliharaan aset lab</p>
                    </div>
                </div>
            </div>
            ...
            <div class="p-6">
                ...
                <form action="{{ route('pemeliharaan.update', $pemeliharaan->id_pm) }}" method="POST" id="formPemeliharaan">
                    @csrf
                    @method('PUT')
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <select id="id_aset_select" name="id_aset_select" required disabled
                                    ...
                                    @foreach($assets as $aset)
                                        <option value="{{ $aset->id_aset }}" data-kode="{{ $aset->kode_brg }}" @selected(old('id_aset', $pemeliharaan->id_aset) == $aset->id_aset)>
                                ...
                                <div class="rounded-xl border border-amber-200 bg-amber-50 p-4 dark:border-amber-500/20 dark:bg-amber-500/10">
                                    <div class="grid gap-3 sm:grid-cols-3">
                                        ...
                                        <div>
                                            ...
                                            <p id="infoUsia" class="mt-0.5 text-sm font-bold text-amber-900 dark:text-amber-100">
                        ...
                        <div class="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                            ...
                            <div>
                                <label for="tgl_pm" class="mb-1.5 block text-sm font-semibold text-slate-700 dark:text-slate-200">
                                    <i class="fa-regular fa-calendar mr-1 text-amber-500"></i> Tanggal PM <span class="text-rose-500">*</span>
                                </label>
                                <input type="date" id="tgl_pm" name="tgl_pm"
                                       value="{{ old('tgl_pm', $pemeliharaan->tgl_pm ? $pemeliharaan->tgl_pm->format('Y-m-d') : '') }}" required
                            ...
                            <div>
                                <label for="jenis_pm" class="mb-1.5 block text-sm font-semibold text-slate-700 dark:text-slate-200">
                                    <i class="fa-solid fa-list-check mr-1 text-amber-500"></i> Jenis PM <span class="text-rose-500">*</span>
                                </label>
                                <select id="jenis_pm" name="jenis_pm" required
                                    ...
                                    <option value="Preventif" @selected(old('jenis_pm', $pemeliharaan->jenis_pm) === 'Preventif')>Preventif</option>
                                    <option value="Korektif" @selected(old('jenis_pm', $pemeliharaan->jenis_pm) === 'Korektif')>Korektif</option>
                                    <option value="Tidak Ada" @selected(old('jenis_pm', $pemeliharaan->jenis_pm) === 'Tidak Ada')>Tidak Ada</option>
                                </select>
                                @error('jenis_pm')
                                    <p class="mt-1 text-xs font-medium text-rose-600 dark:text-rose-400">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                <label for="interval_bulan" class="mb-1.5 block text-sm font-semibold text-slate-700 dark:text-slate-200">
                                    <i class="fa-solid fa-arrows-spin mr-1 text-amber-500"></i> Interval (Bulan) <span class="text-rose-500">*</span>
                                </label>
                                <input type="number" id="interval_bulan" name="interval_bulan"
                                       value="{{ old('interval_bulan', $pemeliharaan->interval_bulan) }}" min="0" required
                            ...
                            <div>
                                <label for="pelaksana" class="mb-1.5 block text-sm font-semibold text-slate-700 dark:text-slate-200">
                                    <i class="fa-solid fa-user-gear mr-1 text-amber-500"></i> Pelaksana <span class="text-rose-500">*</span>
                                </label>
                                <select id="pelaksana" name="pelaksana" required
                                    ...
                                    <option value="Teknisi Internal" @selected(old('pelaksana', $pemeliharaan->pelaksana) === 'Teknisi Internal')>Teknisi Internal</option>
                                    <option value="Vendor Luar" @selected(old('pelaksana', $pemeliharaan->pelaksana) === 'Vendor Luar')>Vendor Luar</option>
                                    <option value="Guru TKJ" @selected(old('pelaksana', $pemeliharaan->pelaksana) === 'Guru TKJ')>Guru TKJ</option>
                                </select>
                                @error('pelaksana')
                                    <p class="mt-1 text-xs font-medium text-rose-600 dark:text-rose-400">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                <label for="biaya_servis" class="mb-1.5 block text-sm font-semibold text-slate-700 dark:text-slate-200">
                                    <i class="fa-solid fa-rupiah-sign mr-1 text-amber-500"></i> Biaya Servis (Rp) <span class="text-rose-500">*</span>
                                </label>
                                <input type="number" id="biaya_servis" name="biaya_servis"
                                       value="{{ old('biaya_servis', intval($pemeliharaan->biaya_servis)) }}" min="0" required
                            ...
                            <div>
                                <label for="kon_after" class="mb-1.5 block text-sm font-semibold text-slate-700 dark:text-slate-200">
                                    <i class="fa-solid fa-heart-circle-check mr-1 text-amber-500"></i> Kondisi Akhir <span class="text-rose-500">*</span>
                                </label>
                                <select id="kon_after" name="kon_after" required
                                    ...
                                    <option value="B" @selected(old('kon_after', $pemeliharaan->kon_after) === 'B')>B (Baik)</option>
                                    <option value="RR" @selected(old('kon_after', $pemeliharaan->kon_after) === 'RR')>RR (Rusak Ringan)</option>
                                    <option value="RB" @selected(old('kon_after', $pemeliharaan->kon_after) === 'RB')>RB (Rusak Berat)</option>
                                </select>
                                @error('kon_after')
                                    <p class="mt-1 text-xs font-medium text-rose-600 dark:text-rose-400">{{ $message }}</p>
                                @enderror
                            ...
                            <div class="sm:col-span-2 lg:col-span-3">
                                <label for="ket_pm" class="mb-1.5 block text-sm font-semibold text-slate-700 dark:text-slate-200">
                                    <i class="fa-solid fa-comment-dots mr-1 text-amber-500"></i> Keterangan Pemeliharaan
                                </label>
                                <textarea id="ket_pm" name="ket_pm" rows="3"
                                          placeholder="Catatan pengerjaan servis, penggantian sparepart, dll..."
                                          class="form-control w-full py-2.5 @error('ket_pm') !border-rose-400 !ring-rose-400 @enderror">{{ old('ket_pm', $pemeliharaan->ket_pm) }}</textarea>
                                @error('ket_pm')
                                    <p class="mt-1 text-xs font-medium text-rose-600 dark:text-rose-400">{{ $message }}</p>
                                @enderror
                            </div>
