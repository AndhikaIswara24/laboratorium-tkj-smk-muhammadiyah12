@extends('layouts.app')
@section('title', 'Edit Observasi Efisiensi')
@section('content')
...
<div class="space-y-6">
    ...
    <div class="mx-auto max-w-4xl">
        <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
            {{-- Card Header --}}
            <div class="border-b border-slate-200 bg-gradient-to-r from-amber-500 to-orange-600 px-6 py-5 dark:border-slate-800">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h1 class="text-lg font-bold text-white">Edit Observasi Efisiensi</h1>
                        <p class="text-sm text-amber-100">Perbarui metrik performa, jam operasional, downtime, dan efisiensi output aset</p>
                    </div>
                </div>
            </div>
            ...
            <div class="p-6">
                ...
                <form action="{{ route('efisiensi.update', $efisiensi->id_efisiensi) }}" method="POST" id="formEfisiensi">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-8">
                        <h3 class="mb-4 flex items-center gap-2 text-sm font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">
                            <span class="flex h-6 w-6 items-center justify-center rounded-md bg-amber-100 text-xs text-amber-600 dark:bg-amber-500/10 dark:text-amber-400">1</span>
                            Informasi Aset
                        </h3>
                                ...
                                <label for="id_aset" class="mb-1.5 block text-sm font-semibold text-slate-700 dark:text-slate-200">
                                 ...
                                 <select id="id_aset_select" name="id_aset_select" required disabled
                                     ...
                                     @foreach($assets as $aset)
                                         <option value="{{ $aset->id_aset }}" data-kode="{{ $aset->kode_brg }}" @selected(old('id_aset', $efisiensi->id_aset) == $aset->id_aset)>
                                             {{ $aset->kode_brg }} — {{ $aset->nama_brg }} ({{ $aset->merk_tipe ?? '-' }})
                                         </option>
                                     @endforeach
                                 </select>
                                 <input type="hidden" name="id_aset" value="{{ $efisiensi->id_aset }}">
                            ...
                            <div id="assetInfoCard" class="hidden sm:col-span-2">
                                <div class="rounded-xl border border-amber-200 bg-amber-50 p-4 dark:border-amber-500/20 dark:bg-amber-500/10">
                                    <div class="grid gap-3 sm:grid-cols-3">
                                        <div>
                                            <p class="text-xs font-semibold uppercase text-amber-600 dark:text-amber-300">Kode Barang</p>
                                            <p id="infoKode" class="mt-0.5 text-sm font-bold text-amber-900 dark:text-amber-100">-</p>
                                        </div>
                                        <div>
                                            <p class="text-xs font-semibold uppercase text-amber-600 dark:text-amber-300">Tahun Perolehan</p>
                                            <p id="infoTahun" class="mt-0.5 text-sm font-bold text-amber-900 dark:text-amber-100">-</p>
                                        </div>
                                        <div>
                                            <p class="text-xs font-semibold uppercase text-amber-600 dark:text-amber-300">Usia Pakai</p>
                                            <p id="infoUsia" class="mt-0.5 text-sm font-bold text-amber-900 dark:text-amber-100">-</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="mb-8">
                        <h3 class="mb-4 flex items-center gap-2 text-sm font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">
                            <span class="flex h-6 w-6 items-center justify-center rounded-md bg-amber-100 text-xs text-amber-600 dark:bg-amber-500/10 dark:text-amber-400">2</span>
                        ...
                        <div class="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                            ...
                            <div>
                                <label for="tgl_observasi" class="mb-1.5 block text-sm font-semibold text-slate-700 dark:text-slate-200">
                                    <i class="fa-regular fa-calendar mr-1 text-amber-500"></i> Tanggal Observasi <span class="text-rose-500">*</span>
                                </label>
                                <input type="date" id="tgl_observasi" name="tgl_observasi"
                                       value="{{ old('tgl_observasi', $efisiensi->tgl_observasi ? $efisiensi->tgl_observasi->format('Y-m-d') : '') }}" required
                            ...
                            <div>
                                <label for="jam_ops" class="mb-1.5 block text-sm font-semibold text-slate-700 dark:text-slate-200">
                                    <i class="fa-solid fa-clock mr-1 text-amber-500"></i> Jam Ops / Bulan <span class="text-rose-500">*</span>
                                </label>
                                <input type="number" step="0.1" id="jam_ops" name="jam_ops"
                                       value="{{ old('jam_ops', $efisiensi->jam_ops) }}" min="0" required
                            ...
                            <div>
                                <label for="penggunaan" class="mb-1.5 block text-sm font-semibold text-slate-700 dark:text-slate-200">
                                    <i class="fa-solid fa-chart-line mr-1 text-amber-500"></i> Tingkat Penggunaan <span class="text-rose-500">*</span>
                                </label>
                                <select id="penggunaan" name="penggunaan" required
                                    ...
                                    <option value="Tinggi" @selected(old('penggunaan', $efisiensi->penggunaan) === 'Tinggi')>Tinggi</option>
                                    <option value="Sedang" @selected(old('penggunaan', $efisiensi->penggunaan) === 'Sedang')>Sedang</option>
                                    <option value="Tidak Pakai" @selected(old('penggunaan', $efisiensi->penggunaan) === 'Tidak Pakai')>Tidak Pakai</option>
                                </select>
                                @error('penggunaan')
                                    <p class="mt-1 text-xs font-medium text-rose-600 dark:text-rose-400">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                <label for="jml_user" class="mb-1.5 block text-sm font-semibold text-slate-700 dark:text-slate-200">
                                    <i class="fa-solid fa-users mr-1 text-amber-500"></i> Jumlah Pengguna <span class="text-rose-500">*</span>
                                </label>
                                <input type="number" id="jml_user" name="jml_user"
                                       value="{{ old('jml_user', $efisiensi->jml_user) }}" min="0" required
                            ...
                            <div>
                                <label for="downtime" class="mb-1.5 block text-sm font-semibold text-slate-700 dark:text-slate-200">
                                    <i class="fa-solid fa-hourglass-half mr-1 text-amber-500"></i> Downtime (Jam/Bln) <span class="text-rose-500">*</span>
                                </label>
                                <input type="number" step="0.1" id="downtime" name="downtime"
                                       value="{{ old('downtime', $efisiensi->downtime) }}" min="0" required
                            ...
                            <div>
                                <label for="perform" class="mb-1.5 block text-sm font-semibold text-slate-700 dark:text-slate-200">
                                    <i class="fa-solid fa-gauge mr-1 text-amber-500"></i> Kecepatan / Performa <span class="text-rose-500">*</span>
                                </label>
                                <select id="perform" name="perform" required
                                    ...
                                    <option value="Normal" @selected(old('perform', $efisiensi->perform) === 'Normal')>Normal</option>
                                    <option value="Lambat" @selected(old('perform', $efisiensi->perform) === 'Lambat')>Lambat</option>
                                    <option value="Mati" @selected(old('perform', $efisiensi->perform) === 'Mati')>Mati</option>
                                </select>
                                @error('perform')
                                    <p class="mt-1 text-xs font-medium text-rose-600 dark:text-rose-400">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                <label for="umur_ekonomis" class="mb-1.5 block text-sm font-semibold text-slate-700 dark:text-slate-200">
                                    <i class="fa-solid fa-hourglass-end mr-1 text-amber-500"></i> Umur Ekonomis (Tahun) <span class="text-rose-500">*</span>
                                </label>
                                <input type="number" id="umur_ekonomis" name="umur_ekonomis"
                                       value="{{ old('umur_ekonomis', $efisiensi->umur_ekonomis) }}" min="0" required
                            ...
                            <div>
                                <label for="efi_out" class="mb-1.5 block text-sm font-semibold text-slate-700 dark:text-slate-200">
                                    <i class="fa-solid fa-arrow-up-right-dots mr-1 text-amber-500"></i> Efisiensi Output <span class="text-rose-500">*</span>
                                </label>
                                <select id="efi_out" name="efi_out" required
                                    ...
                                    <option value="Tinggi" @selected(old('efi_out', $efisiensi->efi_out) === 'Tinggi')>Tinggi</option>
                                    <option value="Sedang" @selected(old('efi_out', $efisiensi->efi_out) === 'Sedang')>Sedang</option>
                                    <option value="Rendah" @selected(old('efi_out', $efisiensi->efi_out) === 'Rendah')>Rendah</option>
                                </select>
                                @error('efi_out')
                                    <p class="mt-1 text-xs font-medium text-rose-600 dark:text-rose-400">{{ $message }}</p>
                                @enderror
                    ...
                    <div class="flex items-center justify-end gap-3 border-t border-slate-200 pt-6 dark:border-slate-800">
                        ...
                        <button type="submit" class="btn-primary !bg-amber-600 hover:!bg-amber-700 dark:!bg-amber-600 dark:hover:!bg-amber-700">
